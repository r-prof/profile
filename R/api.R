...
validate_profile <- function(x) {
  ...
  #' This named list has the following components, subsequently referred to as *tables*:
  #' - `meta`
  #' - `sample_types`
  #' - `samples`
  #' - `locations`
  ...
  #' (Components with names starting with a dot are permitted after the required components, but will be ignored.)

  #'
  #' The `meta` table has two character columns, `key` and `value`.
  #' Additional columns with a leading dot in the name are allowed after the required columns.
  ...
  #' It is currently restricted to one row with values `"samples"` and `"count"`, respectively.
  stopifnot(nrow(x$sample_types) == 1)
  stopifnot(x$sample_types$type == "samples")
  stopifnot(x$sample_types$unit == "count")

  #'
  #' The `samples` table has two columns, `value` (integer) and `locations` (list).
  #' Additional columns with a leading dot in the name are allowed after the required columns.
  ...
  #' The `value` column describes the number of consecutive samples for the given location, and must be greater than zero.
  stopifnot(x$samples$value > 0)
  #' Each element of the `locations` column is a tibble with one integer column, `location_id`.
  ...
  #' For each `location_id` value a corresponding observation in the `locations` table must exist.
  stopifnot(unlist(map(x$samples$locations, "[[", "location_id")) %in% x$locations$location_id)
  #' The locations are listed in inner-first order, i.e., the first location corresponds to the innermost entry of the stack trace.

  #'
  #' The `locations` table has three integer columns, `location_id`, `function_id`, and `line`.
  #' Additional columns with a leading dot in the name are allowed after the required columns.
  ...
  #' For each `function_id` value a corresponding observation in the `functions` table must exist.
  #' `NA` values are permitted.
  stopifnot(is.na(x$locations$function_id) | x$locations$function_id %in% x$functions$function_id)
  #' The `line` column describes the line in the source code this location corresponds to, zero if unknown.
  ...
  #' The `name`, `system_name` and `filename` columns describe function names (demangled and mangled),
  #' and source file names for a function.
  #' Both `name` and `system_name` must not contain empty strings.
  stopifnot(x$functions$name != "")
  stopifnot(x$functions$system_name != "")
  #' The `start_line` column describes the start line of a function in its source file, zero if unknown.
  #' All values must be nonnegative.
